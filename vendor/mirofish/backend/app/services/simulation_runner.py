...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Use .venv-oasis Python 3.11 if available (camel-oasis requires <3.13)
            # __file__ is backend/app/services/simulation_runner.py
            # project root is 5 levels up: services → app → backend → mirofish → vendor → prememora
            _project_root = os.environ.get(
                'PREMEMORA_ROOT',
                os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../..'))
            )
            oasis_python = os.path.join(_project_root, '.venv-oasis/bin/python')
            if not os.path.exists(oasis_python):
                oasis_python = sys.executable
            cmd = [
                oasis_python,
                script_path,
            ...
            # Ensure adapter module is on PYTHONPATH for the subprocess
            env['PYTHONPATH'] = f"{_project_root}:{os.path.join(_project_root, 'vendor/mirofish/backend')}"
